...
const logRequest = (req) => {
    ...
    //     fs.writeFileSync('logs.json', JSON.stringify(logs, null, 2));
    // } catch (err) {
    //     console.error('Error handling logs.json', err);
    //     // If reading fails, we can still log the current entry to a new array
    //     try {
    //         const logs = [logEntry]; // Initialize with the current log entry
    //         fs.writeFileSync('logs.json', JSON.stringify(logs, null, 2));
    //     } catch (writeErr) {
    ...
    try {
        ...
        let logs = [];
        //We can check with this built in method from the fs module to avoid the nested try catch block from above
