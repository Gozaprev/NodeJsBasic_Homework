...
// GET ALL chefs with tokens
chefRouter.get("/chefs", authenticateJWT, async (req, res) => {
    const chefs = await chefMongoModel.find();
    res.send(chefs);
});
...
// GET chef BY ID
chefRouter.get("/chefs/:id", authenticateJWT, async (req, res) => {
...
// CREATE chef
chefRouter.post('/chefs', authenticateJWT, async (req, res) => {
    const body = req.body;
...
// DELETE chef
chefRouter.delete("/chefs/:id", authenticateJWT, async (req, res) => {
    const chefID = req.params.id;
